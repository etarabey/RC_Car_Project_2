...
uint8_t adc_func(){ 
    esp_err_t ret_adc; 
    uint32_t ret_num_adc = 0; 
    uint8_t result_adc[EXAMPLE_READ_LEN] = {0}; 
    memset(result_adc, 0xcc, EXAMPLE_READ_LEN); 
    s_task_handle = xTaskGetCurrentTaskHandle();
    ESP_LOGI(GATTS_TAG, "I am passed!");  
    adc_continuous_handle_t handle_adc = NULL; 
    continuous_adc_init(channel, sizeof(channel)/sizeof(adc_channel_t), &handle_adc); 

    adc_continuous_evt_cbs_t cbs = { 
        .on_conv_done = s_conv_done_cb,
    }; 
    ESP_ERROR_CHECK(adc_continuous_register_event_callbacks(handle_adc, &cbs,NULL)); 
    ESP_ERROR_CHECK(adc_continuous_start(handle_adc)); 
    ulTaskNotifyTake(pdTRUE, portMAX_DELAY); 
    ret_adc = adc_continuous_read(handle_adc, result_adc, EXAMPLE_READ_LEN, &ret_num_adc, 0); 
    if(ret_adc == ESP_OK){ 
        adc_digi_output_data_t *p = &result_adc; 
        if(check_valid_data(p)){ 
           
            
            return p -> type2.data; 
            /*#else 
            ESP_LOGI(GATTS_TAG, "Works! Bless up"); 
            return result_adc;  
            #endif */
        }
